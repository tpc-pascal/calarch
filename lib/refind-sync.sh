...
manage_refind_entry() {
    ...
    while IFS=: read -r name kernel initrd; do
        ...
        entry_text+="    loader   /$esp_arch_dir/vmlinuz-$name\n"

        if [ -n "$ucode_name" ]; then
            entry_text+="    initrd   /$esp_arch_dir/$ucode_name\n"
        fi
        entry_text+="    initrd   /$esp_arch_dir/initramfs-$name.img\n"
...
main() {
    ...
    if [ -n "$partuuid" ]; then
        ...
        manage_refind_entry "$esp_mnt" "$partuuid" "$rootflags" "$kernel_params" || \
