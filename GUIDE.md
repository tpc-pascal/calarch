...
> Phiên bản mới nhất: v1.0.10 — Gum TUI engine, live mode post-install, UKI support,
> self-bootstrap first login.
...
> **Toàn bộ quy trình từ Arch ISO đến desktop hoạt động — không cần thao tác
> thủ công. Chỉ cần boot USB, chạy một lệnh duy nhất.**

### Luồng 1 — Bootstrap tự động (khuyên dùng)

Dành cho đa số người dùng. Tự động partition toàn bộ đĩa, cài Arch + rEFInd +
calarch, không cần hỏi gì.

```bash
# 1. Boot Arch USB, kiểm tra mạng
ping archlinux.org
# Nếu cần WiFi: iwctl station wlan0 connect <SSID>

# 2. Chạy bootstrap (tự động làm hết — partition, pacstrap, config, hooks, reboot)
...
| Bước | Mô tả |
|------|-------|
| `bootstrap.sh` | Kiểm tra UEFI + network → auto-detect disk → partition (ESP 512MB + Btrfs root) → pacstrap (linux-zen, rEFInd, gum, ...) → cấu hình (timezone, locale, hostname, user, passwords, rEFInd, kernel params) → tạo `.bash_login` self-bootstrap → in thông tin → reboot |
| Sau reboot — lần đầu login | `.bash_login` tự động: cài git → clone calarch → chạy post-install → god-mode setup. Kiểm tra: `cat /tmp/godmode-setup.log` |

> **Ghi chú:**
> - Username mặc định: `pascal`. Hostname: `cfxz6`. Passwords được tạo ngẫu
>   nhiên và hiển thị trước khi reboot (lưu tại `/root/calarch-credentials.txt`
>   trên ISO).
> - Override bằng biến môi trường: `CALARCH_DISK=/dev/sda CALARCH_USER=foo
>   CALARCH_HOSTNAME=bar CALARCH_ROOT_PASS=abc CALARCH_USER_PASS=xyz
>   bash bootstrap.sh`

---

### Luồng 2 — Subvolume tùy chỉnh (nâng cao)

Dành cho người dùng muốn tự chia Btrfs subvolume hoặc partition thủ công.

```bash
# --- Boot Arch USB, kiểm tra mạng ---
ping archlinux.org
...
# --- Partition thủ công (tuỳ ý) ---
# Ví dụ: ESP + Btrfs root
mkfs.fat -F32 /dev/nvme0n1p1      # ESP
mkfs.btrfs -f /dev/nvme0n1p2      # Root

# --- Tạo subvolumes (tuỳ ý) ---
mount /dev/nvme0n1p2 /mnt
btrfs subvolume create /mnt/@
btrfs subvolume create /mnt/@home
btrfs subvolume create /mnt/@snapshots
# Thêm @cache, @log, @pkg nếu muốn
umount /mnt
...
# --- Chạy bootstrap với disk có sẵn (bỏ qua partition) ---
CALARCH_DISK=/dev/nvme0n1 bash bootstrap.sh

# Hoặc cài thủ công + chạy post-install:
#   pacstrap /mnt base base-devel linux-zen ...
#   genfstab -U /mnt >> /mnt/etc/fstab
#   arch-chroot /mnt  # cấu hình thủ công
#   bash lib/post-install.sh post-install /mnt
```

> **Lưu ý:** Nếu dùng `@cache`/`@log`/`@pkg`, phải mount đúng thứ tự — `@pkg`
> là subvolume con của `@cache`. Xem hướng dẫn Arch Wiki.

------------------------------------------------------------------------

## Sau khi reboot lần đầu

1. **Người dùng:** login bằng user đã tạo
2. **Tự động:** `.bash_login` cài git, clone calarch, chạy post-install +
   god-mode setup (ngầm, không block login)
3. **Người dùng:** kiểm tra trạng thái — `cat /tmp/godmode-setup.log`
4. **Tự động:** nếu setup lỗi, lần login sau sẽ thử lại
5. **Người dùng:** sau khi setup xong, chạy — `cd ~/calarch && bash start.sh`
